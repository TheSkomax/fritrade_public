...
us500_button_xpath = "/html/body/div[2]/div[5]/div/div[1]/div[1]/div[1]/div[1]/div[2]/div/div[2]/div/div/div[2]/div/div[4]/div/div/div[2]/span/span[1]"
vix_button_xpath = "/html/body/div[2]/div[5]/div/div[1]/div[1]/div[1]/div[1]/div[2]/div/div[2]/div/div/div[2]/div/div[16]/div/div/div[2]/span/span[1]"

timeframe_list_us500 = ["1m", "3m", "5m", "15m", "30m", "45m", "1h", "2h", "3h", "4h", "D"]
# timeframe_list_us500 = ["1m", "3m", "5m",]
timeframe_list_vix = ["1m", "3m", "5m", "15m", "30m", "45m", "1h", "2h", "3h", "4h", "D"]
# timeframe_list_vix = ["1m", "3m", "5m",]
...
timeframe_button_xpaths = {
    ...
    "W": "/html/body/div[2]/div[2]/div/div/div[1]/div[1]/div/div/div/div/div[2]/div/div[12]",
    "M": "/html/body/div[2]/div[2]/div/div/div[1]/div[1]/div/div/div/div/div[2]/div/div[13]",
...
class US500:
    def __init__(self):
        ...
        self.title_to_assert_us500 = {"symbol": "US500", "chart_name": "INDEXY Majchl", }
    ...
    def open_browser(self):
        ...
        if self.options.headless:
            print("Opening US500 in HEADLESS mode")
        else:
            print("Opening US500 in VISIBLE mode")
        cookies = get_cookies_values("/home/michal/PycharmProjects/trade/tradingview_us500_cookies.csv")

        self.driver_us500 = webdriver.Firefox(options=self.options, executable_path=self.executable)
        self.driver_us500.get("https://www.tradingview.com/chart")
        print("Adding cookies to US500 driver")
        for cook in cookies:
            self.driver_us500.add_cookie(cook)
        print("Refreshing US500 driver\n-----------------------------------------")
        ...
        self.close_def_tab()
        print("US500 driver successfuly deployed!")

    def open_week_chart(self):
        timeframe = "W"
        self.driver_us500.switch_to.window(handles_vix["D"])
        self.switch_chart_to_timeframe(timeframe)
        self.check_chart(timeframe)
    ...
    def try_to_assert(self):
        ...
        while x == 0:
            try:
                ...
                while y == 0:
                    try:
                        print("Waiting for chart to load completely - trying to assert (selecting US500)")
                        select_us500 = self.driver_us500.find_element(By.XPATH, us500_button_xpath)
                        select_us500.click()
                        y = 1
                    except:
                        print("Failed asserting/selecting US500")
                        time.sleep(3)
        ...
        while not tf_ok:
            try:
                ...
                if number.text == timeframe_numbers[timeframe]:
                    ...
                    print("Timeframe of the chart is", number.text, "...should be", timeframe, "- trying to switch")
                    self.switch_chart_to_timeframe(timeframe)
                    time.sleep(1)
                    if number.text == timeframe_numbers[timeframe]:
                        print("Timeframe", timeframe, "confirmed!")
...
class VIX:
    def __init__(self):
        ...
        self.options.headless = False
        self.executable = "/home/michal/geckodriver-v0.30.0-linux32/geckodriver"
        self.title_to_assert_vix = {"symbol": "VIX", "chart_name": "INDEXY Majchl", }
    ...
    def open_browser(self):
        ...
        if self.options.headless:
            print("Opening VIX in HEADLESS mode")
        else:
            print("Opening VIX in VISIBLE mode")
        cookies = get_cookies_values("/home/michal/PycharmProjects/trade/tradingview_us500_cookies.csv")

        self.driver_vix = webdriver.Firefox(options=self.options, executable_path=self.executable)
        self.driver_vix.get("https://www.tradingview.com/chart")
        print("Adding cookies to VIX driver")
        for cook in cookies:
            self.driver_vix.add_cookie(cook)
        print("Refreshing VIX driver\n-----------------------------------------")
        self.driver_vix.refresh()

        i = 0
        for timeframe in timeframe_list_vix:
            i = i + 1
            print("Opening chart for timeframe", timeframe)
            self.driver_vix.execute_script('window.open("{charturl}")'.format(charturl=chart_url))
            handles_vix.update({timeframe: self.driver_vix.window_handles[i]})
            # print(handles)
            self.driver_vix.switch_to.window(handles_vix[timeframe])

            self.switch_chart_to_timeframe(timeframe)
            self.check_chart(timeframe)

        self.close_def_tab()
        print("VIX driver succesfully deployed!")

    def open_week_chart(self):
        timeframe = "W"
        self.driver_vix.switch_to.window(handles_vix["D"])
        self.switch_chart_to_timeframe(timeframe)
        self.check_chart(timeframe)
    ...
    def close_def_tab(self):
        self.driver_vix.switch_to.window(handles_vix["def"])
        print("Closing def tab")
        self.driver_vix.close()

    def switch_chart_to_timeframe(self, timeframe):
        if timeframe in timeframe_list_vix:
            x = False
            while not x:
                try:
                    self.selected_timeframe = self.driver_vix.find_element(By.XPATH,
                                                                           timeframe_button_xpaths[timeframe])
                    x = True
                except:
                    pass
            while not self.selected_timeframe.is_displayed():
                self.driver_vix.implicitly_wait(1)
                print("!!!!!!!!!!!!!!!!!!!!!! Waiting for", timeframe,
                      "timeframe button to show up... shouldnt need to")
            self.try_to_assert()
            print("Selecting", timeframe, "chart")
    ...
    def check_chart(self, timeframe):
        ...
        val_ok = False
        while not val_ok:
            value = self.driver_vix.find_element(By.XPATH, value_xpath)
            while not value.is_displayed():
                print("Waiting for key value to load on", timeframe, "chart")
                time.sleep(2)
            print("Key value found!")
    ...
    def try_to_assert(self):
        def select_vix():
            ...
            while y == 0:
                ...
                try:
                    button = self.driver_vix.find_element(By.XPATH, vix_button_xpath)
                    button.click()
                    y = 1
                except Exception as e:
                    print("DEBUG - nemozem najst VIX button!!!!!!!!! 3s sleep")
                    time.sleep(3)
        x = 0
        while x == 0:
            try:
                assert self.title_to_assert_vix["symbol"] in self.driver_vix.title
                assert self.title_to_assert_vix["chart_name"] in self.driver_vix.title
                x = 1
                print(self.title_to_assert_vix["symbol"], self.title_to_assert_vix["chart_name"],
                      "in title confirmed!")
            except:
                select_vix()
                # while y == 0:
    ...
    def check_timeframe_number(self, timeframe):
        ...
        while not tf_ok:
            try:
                number = self.driver_vix.find_element(By.XPATH, timeframe_number_xpath)
                if number.text == timeframe_numbers[timeframe]:
                    print("Timeframe", timeframe, "confirmed!")
                    tf_ok = True
                # else:
...
class valuegrabber_US500:
    def __init__(self):
        ...
        # ak v USA je aktivne DST
        if dst_check(datetime(date.today().year, date.today().month, date.today().day), timezone="US/Pacific"):
    ...
    def reinitialize_init_variables(self):
        # toto je kvoli tomu americkemu posunu casu DST
    ...
    def right_time_finder(self):
        while True:
            ...
            if len(self.key_val) != 0:
                ...
                symbols["US500"].update(self.key_val)
...
class valuegrabber_vix:
    def __init__(self):
        self.target_secs = 57
        self.target_mins = 59

        # zoznamy su vzdy o 1 nizsie ako na trading view, aby sa hodnota kontrolovala este v danej sviecke
        # TODO: tieto su upravene podla tradingview kde su tie casy trochu posunute.. neni cela hodina ale 9:10 napr
        # TODO: bude lepsie pouzit tieto alebo co su klasicke na Sape?
        # TODO: Uvidime ako to bude so zmenou VOLX / VIX na XTB - ktory instrument na tradingview
        # TODO: tomu bude podobny - aktualne som to daval podla VOLX - Mini VIX Future
        self.three_min_list = [2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32, 35, 38, 41, 44, 47, 50, 53, 56, 59]
        self.five_min_list = [4, 9, 14, 19, 24, 29, 34, 39, 44, 49, 54, 59]
        self.fifteen_min_list = [14, 29, 44, 59]
        self.thirty_min_list = [29, 59]
    ...
    def onehour(self):
        if (self.day in self.day_range and
                ...
                self.secs == self.target_secs):
            print("---", time_now())
            self.key_val.update({"1h": vix_browser.get_key_value(timeframe="1h")})
    ...
    def thirtymin(self):
        if (self.day in self.day_range and
                ...
                self.secs == self.target_secs):
            print("---", time_now())
            self.key_val.update({"30m": vix_browser.get_key_value(timeframe="30m")})
    ...
    def fortyfivemin(self):
        if (self.day in self.day_range and
                ...
                self.secs == self.target_secs):
            print("---", time_now())
            self.key_val.update({"45m": vix_browser.get_key_value(timeframe="45m")})
    ...
    def onemin(self):
        ...
        if (self.day in self.day_range and
                ...
                self.secs == self.target_secs):
            print("---", time_now())
            self.key_val.update({"1m": vix_browser.get_key_value(timeframe="1m")})
    ...
    def threemin(self):
        if (self.day in self.day_range and
                ...
                self.secs == self.target_secs):
            print("---", time_now())
            self.key_val.update({"3m": vix_browser.get_key_value(timeframe="3m")})
    ...
    def fivemin(self):
        if (self.day in self.day_range and
                ...
                self.secs == self.target_secs):
            print("---", time_now())
            self.key_val.update({"5m": vix_browser.get_key_value(timeframe="5m")})
    ...
    def fifteenmin(self):
        if (self.day in self.day_range and
                ...
                self.secs == self.target_secs):
            print("---", time_now())
            self.key_val.update({"15m": vix_browser.get_key_value(timeframe="15m")})
    ...
    def twohour(self):
        if (self.day in self.day_range and
                ...
                self.secs == self.target_secs):
            print("---", time_now())
            self.key_val.update({"2h": vix_browser.get_key_value(timeframe="2h")})
    ...
    def threehour(self):
        if (self.day in self.day_range and
                ...
                self.secs == self.target_secs):
            print("---", time_now())
            self.key_val.update({"3h": vix_browser.get_key_value(timeframe="3h")})
    ...
    def fourhour(self):
        if (self.day in self.day_range and
                ...
                self.secs == self.target_secs):
            print("---", time_now())
            self.key_val.update({"4h": vix_browser.get_key_value(timeframe="4h")})
    ...
    def oneday(self):
        if (self.day in self.day_range and
                ...
                self.secs == self.target_secs):
            print("---", time_now())
            self.key_val.update({"D": vix_browser.get_key_value(timeframe="D")})
    ...
    def week(self):
        ...
        if (check_current_day() == 4 and
                time_now() == target_time):
            print("---", time_now())
            vix_browser.open_week_chart()

            self.key_val.update({"W": vix_browser.get_key_value(timeframe="D")})
    ...
    def right_time_finder(self):
        while True:
            ...
            if len(self.key_val) != 0:
                print("*** VIX key vals:", self.key_val)
                # self.value_writer()

                symbols["VIX"].update(self.key_val)
...
# TODO: Aka funkcia bude spustat breakfinder.look_for_red_wave_break????
class Breakfinder:
    def look_for_wave_buy_break(self, symbol, timeframe):
        ...
        qpart1 = "select key_value from fri_trade."
        qpart2 = "_"
        ...
        try:
            ...
            if buy and condition is not None:
                ...
                # TODO: a na ktorom timeframe bol zlom
                ...
                xtb_trader.trader_buy(symbol=symbol, timeframe=timeframe)

            else:
                print("{symbol} {tf} - čakám na obchodnú príležitosť".format(symbol=symbol, tf=timeframe))

        except IndexError:
            print("{symbol} {tf} - čakám na obchodnú príležitosť - nedostatok hodnôt v databáze".format(symbol=symbol,
                                                                                                        tf=timeframe))

        # TODO: pridat stlpec  databazy, kde sa bude zapisovat buy/sell/nič + den slovom?... potom to nejak prepojit, ze
        # TODO: k jednotlivym zapisom s buy/sell bude aj pridavat realizovany zisk?.. ze to nejak bude vediet vybraz z xAPI?
            ...
            for symbol, value in symbols.items():
                if len(value) != 0:
                    ...
                    for tf in value:
                        ...
                        qpart2 = "_"
                        qpart3 = " (key_value, dateOfValue, timeOfValue) VALUES(%s, %s, %s)"
                        q = qpart1 + symbol + qpart2 + tf + qpart3

                        fri_trade_cursor.execute(q, (value[tf], date_now(), time_now(),))
                        print("Key value from {symb} {tf} added to database!".format(symb=symbol, tf=tf))

                        Breakfinder.look_for_wave_buy_break(symbol=symbol, timeframe=tf)
