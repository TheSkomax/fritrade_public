...
def open_trade(chart_name, indicator, operation, takeprofit_pips, stoploss_pips):
    ...
    for opened_trade in allopenedtrades:
        if opened_trade["order2"] == ordernum:
            ...
            q = f"""insert into fri_trade.positions (date, time, positionnum, ordernum, symbol, ordertype, lots, margin, conditionTriggered, timeframe, sent, opened, reason, protected) VALUES('{date_now()}', '{time_now_hms()}', '{positionnum}', '{ordernum}', '{symbol}', '{operation}', '{lots}', '{margin_needed_for_new_trade}', '{indicator}', '{timeframe}', '{sent}', '{opened}', '{message}', False)"""
            fri_trade_cursor.execute(q)
...
open_trade(chart_name=argv[1],
           indicator=argv[2],
           operation=argv[3],
           takeprofit_pips=argv[4],
           stoploss_pips=argv[5])
