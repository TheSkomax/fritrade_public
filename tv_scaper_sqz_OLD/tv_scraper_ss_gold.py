...
symbol_select_button_xpath = "/html/body/div[3]/div[6]/div/div[1]/div[1]/div[1]/div[1]/div[2]/div/div[2]/div/div/div[2]/div/div[6]/div/div/div[2]/span/span[2]"
...
# ====== browser logging ======
log_browser = logging.getLogger("GOLD_browser_logger")
...
# ====== writing logging ======
log_writing = logging.getLogger("GOLD_writing_logger")
...
def mysql_keepalive():
    while True:
        q = "select id from fri_trade.GOLD_1h order by id desc limit 1"
...
class Trading:
    def __init__(self):
        ...
        self.driver = None
        self.title_to_assert = {"symbol": symbol,
    ...
    def right_time_finder(self):
        while True:
            ...
            if len(self.key_val) != 0:
                ...
                active_symbol.update(self.key_val)
                symbol_atrb[symbol].update(self.atrb_values)
                threads_done.update({symbol: True})
...
def writer_new():
    try:
        ...
        """HLAVNY CYKLUS========================================================"""
        while True:
            symbol_thread_done = threads_done[symbol]

            # ========================  GOLD  ========================
            if symbol_thread_done:
                symbol_values = active_symbol
                if len(symbol_values) != 0:
                    out = f"\n=========  {symbol} results {time_now()} {date_now()}  ========="
                    print(out)
                    # log_writing.info(out)
                    ...
                    for registered_timeframe in symbol_values:
                        ...
                        # DEBUG
                            ...
                            log_writer.critical(f"{error} {registered_timeframe} {symbol}")
                            print("Error:", error)
                    symbol_values.clear()
...
righttimefinder_thread = threading.Thread(target=grabber.right_time_finder,
                                          name="righttimefinder_gold_thread",
                                          daemon=True)
